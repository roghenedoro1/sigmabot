...
def get_5min_data_sync(symbol): # Run this in thread
    try:
...
async def get_5min_data(symbol):
    return await asyncio.to_thread(get_5min_data_sync, symbol) # KEY FIX

async def generate_forest_signal(context, chat_id):
    await context.bot.send_message(chat_id=chat_id, text="🌲 Scanning 4 pairs... 20s")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text('✅ Bot is LIVE! Welcome to Forest Signalbot! 👋', reply_markup=InlineKeyboardMarkup(keyboard))

async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query; await query.answer(); data = query.data
    if data == 'signal':
        await query.edit_message_text("🌲 Scanning markets... Please wait 20s")
...
async def send_signals(context: ContextTypes.DEFAULT_TYPE):
    await generate_forest_signal(context, YOUR_ID)
...
def main():
    ...
    app.job_queue.run_repeating(send_signals, interval=600, first=60) # first signal after 60s
